...
public class Util {
    ...
    public static File writeFile(String fileName, byte[] data) {
        ...
        try {
            ...
            log("Can not write file:"+fileName);
            return null;
        }
    }
    ...
    public static Object getProperty(Object bean, String field) {
        ...
        try {
            ...
            if (!field.equals("ln4"))
            log(String.format("Can not get %s with class name %s",field,sClass));
              return null;
        }
    }
    ...
    public static void setProperty(Object bean, String field, Object value) {
        ...
        try {
            ...
            log(String.format("Can not set %s with class name %s",field,sClass));
        }
    }
    ...
    public static String lcap(String s) {
        if (isBlank(s)) return s;
        String ret = s.substring(0, 1).toLowerCase() + s.substring(1);
        return ret;
    }
    ...
    public static String getSecond(String text, String pattern) {
        String[] sa = split(text, pattern);
        return sa.length == 1 ? sa[0] : sa[1];
    }

    public static String getLeft(String text, String pattern) {
        String last = getLast(text, pattern);
        int n = text.indexOf(last);
        String rest = text.substring(0, n);
        return rest;
    }

    public static String getRight(String text, String pattern) {
        text=text.trim();
    ...
    */
    ...
    public static void printHash(Map<String, String> table) {
        log("---------------------------------------------");
        for (String key : table.keySet())
            log(key + "\t=" + table.get(key));
    }
    
    public static void printBHash(Map<String,XmlBody> table) {
        log("---------------------------------------------");
        for (String key : table.keySet())
            log(key + "\t=" + table.get(key).body());
    }
    ...
    private static void getFiles(ArrayList list,File file){
        File[] listFile=file.listFiles();
        for (File f:listFile){
            if (f.isDirectory()) getFiles(list,f);
            else list.add(f);
        }
    }
